...
export default function Soc2HubPage() {
  // Deterministic rotation based on current day of the year/week to keep it "dynamic" but stable.
  const featuredIndex = new Date().getDay() % SOC2_TOOLS.length;
  const featuredTool = SOC2_TOOLS[featuredIndex];
  const remainingTools = SOC2_TOOLS.filter((_, i) => i !== featuredIndex);
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-20 text-center space-y-6">
          ...
          <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              ...
              className="inline-flex items-center gap-2 bg-brand-600 hover:bg-brand-700 text-white font-bold px-8 py-4 rounded-xl shadow-lg hover:shadow-xl transition-all"
            >
              {featuredTool.cta}
