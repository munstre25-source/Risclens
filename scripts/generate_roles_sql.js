...
const sqlValues = roles.map(([name, slug, desc]) => {
    let content = {
        roleName: name,
    ...
    return `('${slug}', '${title}', '${meta}', '${JSON.stringify(content).replace(/'/g, "''")}', 'role')`;
});

console.log("INSERT INTO pseo_pages (slug, title, meta_description, content_json, category)");
console.log("VALUES");
