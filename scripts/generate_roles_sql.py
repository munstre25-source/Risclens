...
for name, slug, desc in roles:
    content = {
        ...
        "keyPriorities": [
            {"title": f"Priority 1 for {name}", "description": f"Detailed description of priority 1 for the {name} role in the context of SOC 2."},
            {"title": f"Priority 2 for {name}", "description": f"Detailed description of priority 2 for the {name} role in the context of SOC 2."},
            {"title": f"Priority 3 for {name}", "description": f"Detailed description of priority 3 for the {name} role in the context of SOC 2."},
            {"title": f"Priority 4 for {name}", "description": f"Detailed description of priority 4 for the {name} role in the context of SOC 2."}
        ],
        "faqs": [
            {"question": f"Question 1 for {name}?", "answer": f"Answer 1 specifically tailored for the {name} role."},
            {"question": f"Question 2 for {name}?", "answer": f"Answer 2 specifically tailored for the {name} role."},
            {"question": f"Question 3 for {name}?", "answer": f"Answer 3 specifically tailored for the {name} role."}
        ],
        "relatedLinks": [
    ...
    # Custom priorities/FAQs for existing roles if available
    if slug == "cto":
    ...
    title = f"SOC 2 Compliance for {name}s | RiscLens"
    meta = f"A strategic guide to SOC 2 compliance for {name}s. Learn how to manage the technical audit without slowing down development."
    
    sql_values.append(f"('{slug}', '{title}', '{meta}', '{json.dumps(content).replace(\"'\", \"''\")}', 'role')")

print("INSERT INTO pseo_pages (slug, title, meta_description, content_json, category)")
print("VALUES")
print(",\n".join(sql_values))
print("ON CONFLICT (slug) DO UPDATE SET")
print("title = EXCLUDED.title,")
print("meta_description = EXCLUDED.meta_description,")
print("content_json = EXCLUDED.content_json,")
