              ...
              <div className="mt-8 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm">
                ...
                {relatedLinks.map((link, idx) => (
                  <Link 
                    key={idx} 
                    href={link.href} 
                    className="text-brand-700 hover:text-brand-800 underline underline-offset-4 decoration-slate-200 hover:decoration-brand-300"
                  >
                    {link.label.replace('Related: ', '').replace('Related: Penetration Testing', 'Pentesting').replace('Related: Vendor Risk Assessment', 'Vendor Risk')}
                  </Link>
                ))}
              </div>
            )}
